...
function updateSuggestions(matches: string[]): void {
  ...
  const suggestions = document.getElementById('suggestions');
  if (suggestions != null) {
    matches.forEach(element => suggestions.innerHTML += '<li tabindex="0">' + element);
  }
}
...
document.addEventListener('keyup', listener => {
  if (listener.code === 'Enter') {
    updateValues('Scorpio', 'Taurus', 'Capricorn').then(r => console.log(r)).then(() =>
        postAndUpdate());
  }
})
